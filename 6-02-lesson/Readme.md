...
>  **Hash Join** - если соответствие найдена, то выводиться результирующая строка, иначе строка отбрасывается
>  _**Hash Cond** - для каждой строки clients вычисляется **Hash**, который сравнивается с **Hash** таблицы orders по условию **Hash Cond**.  
>  __-> **Sec Scan on clients** - при выполнении запроса последовательно считывается каждая запись таблицы clients
>  __-> **Hash** - для каждой строки orders вычисляется ее **Hash**.
>  ____-> **Seq Scan on orders o** - при выполнении запроса последовательно считывается каждая запись таблицы orders. 
