...
resource "local_file" "inventory1" {
  content = <<-DOC
    # Hosts to be added to known_hosts.
    # Generated by Terraform.

    [active]
    ${yandex_compute_instance.node01.network_interface.0.nat_ip_address}
    ${yandex_compute_instance.node02.network_interface.0.nat_ip_address}
    ${yandex_compute_instance.node03.network_interface.0.nat_ip_address}

    [workers]
    ${yandex_compute_instance.node04.network_interface.0.nat_ip_address}
    ${yandex_compute_instance.node05.network_interface.0.nat_ip_address}
    ${yandex_compute_instance.node06.network_interface.0.nat_ip_address}
    DOC
  filename = "../ansible/inventory1"
